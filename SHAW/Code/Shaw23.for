...
C
      PROGRAM SHAW
C
C     THIS IS THE DRIVER FOR THE SHAW MODEL (VERSION 2.3.6)
C
C     Last Modified:  September 27, 2004
C
C     DEVELOPED BY: GERALD N. FLERCHINGER
C                   USDA - ARS
C                   800 PARK BLVD., SUITE 105
C                   BOISE, IDAHO 83712
C                   TELEPHONE: 208-442-0716
C                   E-MAIL: gflerchi@nwrc.ars.usda.gov
C
C***********************************************************************
C
...
C        INPUT SOIL MOISTURE IS MATRIC POTENTIAL
      ...
      SOITMP(HOUR)=TSDT(NS)
C
      CALL DAYINP (JULIAN,YEAR,MAXJUL,HOUR,NHRPDT,MTSTEP,
...
C
C     SET FLAG INDICATING FIRST TIME INTO GOSHAW FOR CURRENT PROFILE
      INITAL=0
C
C
...
C        START A NEW DAY
         ...
         CALL DAYINP (JULIAN,YEAR,MAXJUL,0,NHRPDT,MTSTEP,
     >    MPLTGRO,MWATRXT,NS,ALATUD,HRNOON,SUNHOR,TMPDAY,WINDAY,HUMDAY,
     >    PRECIP,SNODEN,SOITMP,VLCDAY,SOILXT,NPLANT,PLTHGT,DCHAR,PLTWGT,
     >    PLTLAI,ROOTDP)
         CALL CLOUDY (CLOUDS,ALATUD,DECLIN,HAFDAY,SUNHOR,JULIAN,NHRPDT)
...
C
     ...
     >     QSV(50),QSVT(50),QSVDT(50),XTRACT(50),US(50),SS(50),
...
C           DEFINE LAYERING OF CANOPY AND ROOT DISTRIBUTION
            CALL CANOPY (NPLANT,NC,NSP,0,MZCINP,ITYPE,PLTHGT,PLTWGT,
     >      PLTLAI,RLEAF0,ZSP,ZC,TC,TCDT,VAPC,VAPCDT,WCAN,WCANDT,WCMAX,
     >      PCANDT,CANMA,CANMB,TMPDAY,HUMDAY)
...
C        INITIALIZE THE WATER BALANCE SUMMARY
     ...
     >   ITYPE,ZC,WCAN,WCANDT,PCAN,PCANDT,VAPC,VAPCDT,RHOSP,DZSP,DLWDT,
     >   WLAG,STORE,ZR,GMC,GMCDT,VAPR,VAPRDT,RHOR,ZS,VLC,VLCDT,VIC,
     >   VICDT,TOTFLO,PRECIP,RUNOFF,POND,EVAP1,ETSUM)
C
C        PRINT OUT INITIAL CONDITIONS
         CALL OUTPUT (NPLANT,NC,NSP,NR,NS,LVLOUT,0,INPH2O,JULIAN,
     >   HRSTRT,YEAR,ZC,TCDT,VAPCDT,WCANDT,ROOTXT,RHOSP,ZSP,TSPDT,
     >   DLWDT,ZR,TRDT,VAPRDT,GMCDT,ZS,TSDT,VLCDT,VICDT,MATDT,TOTFLO,
     >   CONCDT,SALTDT)
C
C        PRINT OUT INITIAL FROST AND SNOW DEPTH
         IF (LVLOUT(10) .NE. 0) CALL FROST (NSP,NS,LVLOUT(10),JULIAN,
     >   HRSTRT,YEAR,ZSP,ZS,VLCDT,VICDT,ICESDT)
C
      ELSE 
        IF (NPLANT .GT. 0) THEN
C        DEFINE LAYERING OF CANOPY AND ROOT DISTRIBUTION
         CALL CANOPY (NPLANT,NC,NSP,0,MZCINP,ITYPE,PLTHGT,PLTWGT,PLTLAI,
     >        RLEAF0,ZSP,ZC,TC,TCDT,VAPC,VAPCDT,WCAN,WCANDT,WCMAX,
     >        PCANDT,CANMA,CANMB,TMPDAY,HUMDAY)
C        DETERMINE ROOT DISTRIBUTION (IF NOT DEFINED BY USER)
         IF (MZCINP.NE.1 .AND. NC.GT.0) 
     >        CALL RTDIST (NPLANT,ITYPE,NS,ZS,ROOTDP,RROOT0)
...
C**** DETERMINE THE AVERAGE VAPOR FLUX BETWEEN SOIL NODES
      IF (ITER .EQ. 1) CALL QVSOIL (NS,QSVT,ZS,TS,MAT,VLC,VIC,CONC)
      CALL QVSOIL (NS,QSVDT,ZS,TSDT,MATDT,VLCDT,VICDT,CONCDT)
...
C-----------------------------------------------------------------------
     ...
     >    JULIAN,HOUR,YEAR,ZC,TCDT,VAPCDT,WCANDT,ROOTXT,RHOSP,ZSP,TSPDT,
     >    DLWDT,ZR,TRDT,VAPRDT,GMCDT,ZS,TSDT,VLCDT,VICDT,MATDT,TOTFLO,
     >    CONCDT,SALTDT)
      END IF
C-----------------------------------------------------------------------
...
C**** DETERMINE THE VAPOR FLUX BETWEEN NODES
      ...
      CALL QVSOIL (NS,QSVDT,ZS,TSDT,MATDT,VLCDT,VICDT,CONCDT)
C
C**** DETERMINE THE AVERAGE VAPOR FLUX OVER THE TIME STEP
      CALL WEIGHT (NS-1,QSV,QSVT,QSVDT)
C
      CALL WBSOIL (N,NS,ZS,MAT,MATDT,VLC,VLCDT,VIC,VICDT,ICESDT,
     >             QSL,QSV,XTRACT,US,ITER)
C
C-----------------------------------------------------------------------
      IF (LEVEL(1) .GE. 2) THEN
         WRITE (21,*) ' VAPOR FLUXES (KG/S)'
...
C              CONTENT FOR THE DIFFERENCE, IF NOT GREATER THAN VLCDT(I)
            ...
            END IF
          ELSE
            IF (ABS(DELTA(N)/MATDT(I)).GT.100. .AND. NDT.LT.MAXNDT) THEN
C              TOO LARGE OF A CHANGE -- CONVERGENCE WILL NOT LIKELY BE
C              BE MET WITH CURRENT TIME STEP.  CUT TIME STEP IN HALF
               IWFLAG=1
               ITER=11
               GO TO 350
            END IF
            IF (ABS(DELTA(N)/MATDT(I)) .GT. TOLER) IWFLAG = IWFLAG+1
            IF (ITER .GT. 3 .AND. DELWTR(N)*DELTA(N) .LT. 0) THEN
C              DELTA IS JUMPING BETWEEN NEG. AND POS.--CUT DELTA IN HALF
C              TO SUPPRESS TENDENCY TO JUMP AROUND SOLUTION
               DELTA(N)=DELTA(N)/2.
            ...
            IF (MATDT(I) .GT. ENTRY(I)) MATDT(I)=ENTRY(I)
         END IF
...
C     SUM THE NECESSARY FLUXES OCCURRING OVER THE TIME STEP
     ...
     >     TRNSP,XTRACT,TSWSNO,TLWSNO,TSWCAN,TLWCAN,TSWRES,TLWRES,
     >     TSWSOI,TLWSOI,THFLUX,EVAP1,ETSUM,ROOTXT,TOTFLO,NTIMES,
     >     TOPSNO,TQVSP)
...
C              ATTEMPT TO INCREASE LENGTH OF SUB-TIME-STEP
         ...
         GO TO 110
        ELSE
C        END OF THE HOUR - INFILTRATE RAIN, DETERMINE OUTFLOW FROM SNOW,
C        AND CALCULATE WATER BALANCE.  SET TIME STEP TO ONE HOUR FOR
C        PRECIP CALULATIONS
         DT = NHRPDT*3600.
         RAIN=PRECIP
         TADT=TMPDAY
         HUMDT=HUMDAY
...
C        ADJUST DEPTHS OF CANOPY LAYERS FOR ANY CHANGE IN SNOWPACK
     ...
     >       ITYPE,PLTHGT,PLTWGT,PLTLAI,RLEAF0,ZSP,ZC,TC,TCDT,BVAPC,
...
C        PRINT OUT WATER BALANCE FOR THIS HOUR
     ...
     >      ITYPE,ZC,BWCAN,WCANDT,BPCAN,PCANDT,BVAPC,VAPCDT,RHOSP,DZSP,
...
C        PRINT OUT ENERGY BALANCE FOR THIS HOUR
         IF (LVLOUT(6) .NE. 0)
     >     CALL ENERGY (NSPLST,LVLOUT(6),JULIAN,HOUR,YEAR,TSWSNO,TLWSNO,
     >      TSWCAN,TLWCAN,TSWRES,TLWRES,TSWSOI,TLWSOI,THFLUX,EVAP1)
C
C        PRINT OUT FROST AND SNOW DEPTH
         IF (LVLOUT(10) .NE. 0) CALL FROST (NSP,NS,LVLOUT(10),JULIAN,
     >                          HOUR,YEAR,ZSP,ZS,VLCDT,VICDT,ICESDT)
C
...
C     1 = DAILY;   2 ==> DATA MATCHES NHRPDT
      IF (MTSTEP.EQ.1) THEN
         CALL DAY2HR (JULIAN,YEAR,SUNHOR,TMPDAY,WINDAY,HUMDAY,
     >                PRECIP,SNODEN,ALATUD,HRNOON)
         NSTART=1
         NSTEP=1
         GO TO 25
      END IF
...
C        SUBROUTINE 'OUTPUT' CAN PRINT IT OUT WITH PREDICTED VALUES
         IF (IFIRST .NE. 0) THEN
C           IF IFIRST =0, VLCMES ALREADY SET IN 'INPUT'
            MEASHR = LVLCHR
            MEASDY = LVLCDY
            DO 65 I=1,NS
               VLCMES(I) = VLC1(I)
...
C***********************************************************************
C
      SUBROUTINE DAY2HR (JULIAN,YEAR,SUNHOR,TMPDAY,WINDAY,
...
C
     ...
     >     PRECIP(24),DYPREC,SNODEN(24)
      ...
      SAVE IFIRST,TMAX1,TMAX,TMIN,TDEW,WIND,DYPREC,SOLRAD,JDAY,JYR
      DATA PI/3.14159/ DTMIN/0.5/ IFIRST/0/
C
      IF (IFIRST.EQ.0) THEN
         TMAX=-999.
...
C***********************************************************************
C
      SUBROUTINE CANOPY (NPLANT,NC,NSP,NSPLST,MZCINP,ITYPE,PLTHGT,
     >      PLTWGT,PLTLAI,RLEAF0,ZSP,ZC,TC,TCDT,VAPC,VAPCDT,WCAN,WCANDT,
     >      WCMAX,PCANDT,CANMA,CANMB,TA,HUMID)
C
...
C                 CHECK IF PLANT HAS ANY LEAF AREA IN THIS LAYER
                  IF (CANLAI(J,I) .LE. 0.0) GO TO 15
                  IF (FRACTN .GT. 0.999) PEVAP(I)=0.0
                  TLC(J,I) = AVGTMP(I)
                  HUMID(I)=1.0
C                 CALCULATE AVERAGE CONDITIONS IN CANOPY FOR AN INTIIAL
C                 APPROXIMATION TO PLEAF AND PXYLEM
...
C              CALCULATE UPDATED ET AND XYLEM POTENTIAL
               ...
               IF (ABS(DELTA/PLEAF1).GT.0.01 .AND. ITER0.LE.20) THEN
...
C***********************************************************************
C
      SUBROUTINE QVSOIL (NS,QSV,ZS,TS,MAT,VLC,VIC,CONC)
...
C***********************************************************************
     ...
     >                SAND(50),SILT(50),CLAY(50),OM(50)
      ...
      REAL HUMID(50),DVT(50),DVP(50),CONVT(50),CONVP(50)
      REAL LF,LS,LV
C
      DO 20 I=1,NS
         ...
         IF (QSL(I-1).GT.0.0) THEN
            IF (VICDT(I).GT.0.0) THEN
              ...
              ELSE
               POROS=SAT(I)
            END IF
            QMAX=(POROS - VLC(I) - VIC(I))*(ZS(I+1) - ZS(I-1))/2./DT
...
C**** DETERMINE THE COEFFICIENTS FOR THE SURFACE LAYER
C
      A1(N+1) = WDT*(CON(1) + RHOL*CL*QSL(1) + CV*QSV(1))
      B1(N) = B1(N) - WDT*(CON(1) + RHOL*CL*QSL(1) + CV*QSV(1))
     >              - (ZS(2)-ZS(1))*CS(1)/(2.*DT)
      C1(N) = WDT*(CON(1) + RHOL*CL*QSL(1) + CV*QSV(1))
      D1(N) = D1(N)-(CON(1)+RHOL*CL*QSL(1)+CV*QSV(1))*(WT*(TS(1)-TS(2))
     >              + WDT*(TSDT(1)-TSDT(2))) - LV*QSV(1) + SS(1)
     >       - (ZS(2)-ZS(1))/(2.*DT)
...
C**** DETERMINE THE MATRIX COEFFICIENTS FOR THE REST OF THE PROFILE
   20 DO 30 I=N+1,N+NS-2
         J=I-N+1
         A1(I+1)=WDT*(CON(J) + RHOL*CL*QSL(J) +CV*QSV(J))
         B1(I)= -WDT*(CON(J-1)+CON(J)+RHOL*CL*(QSL(J-1)+QSL(J))
     >         + CV*(QSV(J-1)+QSV(J))) - (ZS(J+1)-ZS(J-1))*CS(J)/(2.*DT)
         C1(I)=WDT*(CON(J) + RHOL*CL*QSL(J) + CV*QSV(J))
         D1(I)=(CON(J-1) + RHOL*CL*QSL(J-1) + CV*QSV(J-1))
     >            *(WT*(TS(J-1)-TS(J)) + WDT*(TSDT(J-1)-TSDT(J)))
     >        -(CON(J) + RHOL*CL*QSL(J) + CV*QSV(J))
...
C
   ...
   10 CONTINUE
      DLDT=(LF/TMP + CNCGAS)/(-B(I)*G*MAT(I)/VLC(I) + CRT)
C
...
C
   ...
   20 CONTINUE
      PTDLDT=-LF/ ((-B(I)*G*MAT(I)/VLC(I) + CRT)**2)
     >      * (B(I)*G*MAT(I)*(B(I)+1)*DLDT/(VLC(I)**2) + CRKQ)
      RETURN
      END
C***********************************************************************
C
...
C     FREEZE.)
   30 IF ((VLC(I)+VIC(I)*RHOI/RHOL).LT.VLC2 .OR. MAT1.GT.ENTRY(I)) THEN
C*       NO ICE IS PRESENT
         ICES(I)=0
...
C***********************************************************************
C
      SUBROUTINE WBSOIL (N,NS,ZS,MAT,MATDT,VLC,VLCDT,VIC,VICDT,ICESDT,
     >                   QSL,QSV,XTRACT,US,ITER)
C
C     THIS SUBROUTINE CALCULATES THE COEFFICIENTS FOR THE SOIL IN THE
C     NEWTON-RAPHSON PROCEDURE OF THE ENERGY BALANCE
...
C
     ...
     >     QSL(50),QSLT(50),QSLDT(50),QSV(50),XTRACT(50),US(50),
     >     HKT(50),HKDT(50),CON(50),CONT(50)
      INTEGER ICESDT(50)
      SAVE QSLT
C
C**** DETERMINE THE LIQUID MOISTURE FLUX BETWEEN NODES
...
C        NO ICE IS PRESENT -- CALCULATE COEFFICIENTS FOR LIQUID BALANCE
         CALL MATVL3 (1,MATDT(1),VLCDT(1),DLDM)
         B2(N)=B2(N) - WDT*CON(1) - (ZS(2)-ZS(1))/(2.*DT)*DLDM
         A2(N+1)= WDT*CON(1)
      END IF
C
...
C           NO ICE IS PRESENT -- CALCULATE COEFF. FOR LIQUID BALANCE
            CALL MATVL3 (J,MATDT(J),VLCDT(J),DLDM)
            B2(I)= -WDT*(CON(J-1)+CON(J))-(ZS(J+1)-ZS(J-1))/(2.*DT)*DLDM
            C2(I-1)=WDT*CON(J-1)
            A2(I+1)=WDT*CON(J)
         END IF
   20 CONTINUE
...
C              EXCESS IS ALL FROZEN IN THIS LAYER.
               TSPDT(I)=TSPDT(I) +(EXCESS*LF)/(CI*WEL)
               WEL=WEL+EXCESS
               RHOSP(I)=RHOL*WEL/DZSP(I)
               ABOVE = ABOVE - EXCESS
               EXCESS=0.0
...
C
     ...
     >                   STORE,SCOUT)
C
C     THIS SUBROUTINE DETERMINES THE SNOW COVER OUTFLOW DURING EACH PERI
...
C
     ...
     >     CS(50)
      INTEGER ICESDT(50)
      REAL INFMAX,INFIL,PSAT(50),VLC9(50),MAT9(50),SATK9(50),SLOSS(10)
C
...
C
     ...
     >  ITYPE,ZC,WCAN,WCANDT,PCAN,PCANDT,VAPC,VAPCDT,RHOSP,DZSP,DLWDT,
     >  WLAG,STORE,ZR,GMC,GMCDT,VAPR,VAPRDT,RHOR,ZS,VLC,VLCDT,VIC,VICDT,
     >  TOTFLO,PRECIP,RUNOFF,POND,EVAP1,ETSUM)
C
...
C***********************************************************************
C
      SUBROUTINE ENERGY (NSP,LVLOUT,JULIAN,HOUR,YEAR,TSWSNO,TLWSNO,
     >           TSWCAN,TLWCAN,TSWRES,TLWRES,TSWSOI,TLWSOI,THFLUX,EVAP1)
C
...
C
     ...
     >     SUMFLO(50),SUMRXT(50)
      ...
      SAVE IFIRST,NOUT,NAVTMP,NAVMAT,NAVVLC,NAVSLT,NAVCON,
...
C     PRINT OUT SOIL TEMPERATURES IF AT DESIRED INTERVAL
      IF (LVLOUT(3) .GT. 0) THEN
         IF (MOD(HOUR,LVLOUT(3)).EQ.0 .OR. IFIRST.EQ.0) THEN
...
C     PRINT OUT SOIL MOISTURE CONTENT IF AT DESIRED INTERVAL
      IF (LVLOUT(4) .GT. 0) THEN
         IF (MOD(HOUR,LVLOUT(4)).EQ.0 .OR. IFIRST.EQ.0) THEN
...
C     PRINT OUT SOIL MATRIC POTENTIAL IF AT DESIRED INTERVAL
      IF (LVLOUT(5) .GT. 0) THEN
         IF (MOD(HOUR,LVLOUT(5)).EQ.0 .OR. IFIRST.EQ.0) THEN
...
C     PRINT OUT WATER FLOW BETWEEN SOIL NODES IF AT DESIRED INTERVAL
      IF (LVLOUT(8) .GT. 0) THEN
         IF (MOD(HOUR,LVLOUT(8)).EQ.0 .AND. IFIRST.NE.0) THEN
...
C     PRINT OUT EXTRACTED BY ROOTS FROM EACH NODE
      IF (LVLOUT(9) .GT. 0 .AND. NPLANT .GT. 0) THEN
         IF (MOD(HOUR,LVLOUT(9)).EQ.0 .OR. IFIRST.EQ.0) THEN
...
C     PRINT OUT SALT CONCENTRATION IF AT DESIRED INTERVAL
      IF (LVLOUT(11) .GT. 0) THEN
        IF (MOD(HOUR,LVLOUT(11)).EQ.0 .OR. IFIRST.EQ.0) THEN
          DO 50 J=1,NSALT
...
C     PRINT OUT SOLUTE CONCENTRATION IF AT DESIRED INTERVAL
      IF (LVLOUT(12) .GT. 0) THEN
        IF (MOD(HOUR,LVLOUT(12)).EQ.0 .OR. IFIRST.EQ.0) THEN
          DO 60 J=1,NSALT
...
C***********************************************************************
C
      SUBROUTINE FROST (NSP,NS,LVLOUT,JULIAN,HOUR,YEAR,
     >                  ZSP,ZS,VLCDT,VICDT,ICESDT)
C
...
C***********************************************************************
C
      REAL ZSP(100),ZS(50),VLCDT(50),VICDT(50)
      INTEGER HOUR,YEAR,ICESDT(50)
C
      SAVE IFIRST,NPRINT,LAST,LDAY,LHOUR,LYR,ZERO,FDEPTH,TDEPTH
      DATA IFIRST/ 0/
C
      IF (IFIRST .EQ. 0) THEN
...
C     PRESENT THIS TIME STEP --> PRINT OUT ZEROES FOR LAST TIME STEP
     ...
     >    WRITE (27,110) LDAY,LHOUR,LYR,ZERO,ZERO,ZERO,(ZERO,I=1,NPRINT)
      LDAY=JULIAN
      LHOUR=HOUR
      LYR=YEAR
C
...
C
      ...
      END IF
C
C     PRINT FROST, THAW AND SNOW DEPTH IN CENTIMETERS
      WRITE (27,110) JULIAN,HOUR,YEAR,TDEPTH*100,FDEPTH*100,SNOW*100,
     >               (VICDT(I), I=1,NPRINT)
      IF (LAST .EQ. 0) WRITE (27,*)
      RETURN
C
  100 FORMAT (/4X,'FROST AND SNOW DEPTH',///,' DAY HR  YR',
     >5X,'THAW   FROST    SNOW      ICE CONTENT OF EACH NODE (M3/M3)',/,
     >'                 CM      CM      CM',7X,20F6.2)
  110 FORMAT (I4,I3,I5,3F8.1,6X,15F6.3)
  120 FORMAT ('  .   .  .      .       .       .')
C
      END
C***********************************************************************
